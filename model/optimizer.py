...
class Create_optimizer():
    def __init__(self, system_param):
        ...
        self.cam_max_lr = system_param["cam_lr_max"]
        self.cam_min_lr = system_param["cam_lr_min"]
    ...
    def get_cam_optimizer(self, cam_rot_delta, cam_trans_delta):
        if self.opt_type == 'cycle':
            lr_kwargs = {
                'max_steps': self.num_steps,
                'lr_delay_steps': 1500,
                'lr_delay_mult': 1e-08,
            }
            self.cam_lr_fn_main = lambda step: self.learning_rate_decay(step, lr_init=self.cam_max_lr, lr_final=self.cam_min_lr, **lr_kwargs)
            l = [{'params': [cam_rot_delta], 'lr': self.cam_max_lr, "name": "cam_rot_delta"},
                {'params': [cam_trans_delta], 'lr': self.cam_max_lr, "name": "cam_trans_delta"}]
            self.cam_optimizer = torch.optim.Adam(l, eps=1e-15)
        return self.cam_optimizer, self.cam_lr_fn_main
    ...
    def update_cam_params(self, cur_step):
        if self.opt_type == 'decay':
            self.cam_lr_fn_main.step() 
        elif self.opt_type == 'cycle':
            # 更新相机位姿学习速率
            cam_learning_rate = self.cam_lr_fn_main(cur_step)
            for param_group in self.cam_optimizer.param_groups:
                param_group['lr'] = cam_learning_rate
        else:
            exit()
