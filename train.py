...
class Model_Trainer():
    def __init__(self, sys_param):
        ...
        # 初始化位姿学习参数
        self.generate_poses_delta()
        self.pose_path = os.path.join(self.output_path, "poses")
        ...
        self.cam_optimizer, self.cam_lr_fn = self.opti_engine.get_cam_optimizer(self.cam_rot_delta, self.cam_trans_delta)
        # 初始化Tensorboard
        if self.tb_mode:
    ...
    def forward(self):
        # 获取训练数据
        imgs_train, c2ws_train_gt, c2ws_train, intrs_train, intrs_inv_train, idx_train, num_train = self.dataset.getTrainData()
        # 随机选择视角
        ...
        with tqdm(total=self.num_steps, desc="Training:", ncols=150) as pbar:
            for step in range(self.num_steps):
                ...
                # 应用位姿扰动
                cam_delta = torch.cat([self.cam_trans_delta[idx], self.cam_rot_delta[idx]], dim=0)
                c2w = torch.inverse(SE3_exp(cam_delta)) @ c2w
                _, H, W = img.shape
                ### 生成光线原点和方向
                ...
                self.opti_engine.update_cam_params(step)
                ### 更新进度条
                pbar.set_postfix({"loss": f"{loss.item():.8f}",
                                  "lr": f"{self.optimizer.param_groups[0]['lr']:.8f}",
                                  "cam_lr": f"{self.cam_optimizer.param_groups[0]['lr']:.8f}"})
                ### 保存模型检查点
                ...
                with torch.no_grad():
                    ...
                    ### 输出相机位姿损失
                    if step % 500 == 0:
                        init_c2ws = c2ws_train.to(self.device)
                        gt_c2ws = c2ws_train_gt.to(self.device)
                        pd_c2ws = []
                        for i in range(self.train_num):
                            tau = torch.cat([self.cam_trans_delta[i], self.cam_rot_delta[i]], dim=0)
                            pd_c2w = torch.inverse(SE3_exp(tau)) @ init_c2ws[i]
                            pd_c2ws.append(pd_c2w)
                        pd_c2ws = torch.stack(pd_c2ws, dim=0)
                        rot_loss = self.compute_Rot(gt_c2ws, pd_c2ws)
                        ate_loss = self.compute_ATE(gt_c2ws, pd_c2ws)
                        print("\n")
                        print("ROT LOSS: {:.6f}".format(rot_loss))
                        print("ATE LOSS: {:.6f}".format(ate_loss))
                        # 可视化调试
                        gt_c2ws_pca = self.dataset.c2ws_pca_train
                        self.transform_mat, self.scale_factor = self.dataset.transform_mat.to(self.device), self.dataset.scale_factor.to(self.device)
                        pd_c2ws_pca = apply_pca(pd_c2ws, self.transform_mat, self.scale_factor)
                        draw_ax = init_show_figure()
                        show_camera_position(intrs_train.detach().cpu(),
                                            gt_c2ws_pca.detach().cpu(),
                                            W,
                                            draw_ax,
                                            color = (0.0,0.6,0.7), 
                                            cam_size=0.08, 
                                            line_w=0.5)
                        show_camera_position(intrs_train.detach().cpu(),
                                            pd_c2ws_pca.detach().cpu(),
                                            W,
                                            draw_ax,
                                            color = (0.7,0.2,0.7),
                                            cam_size=0.08, 
                                            line_w=0.5)
                        fg_output_path = os.path.join(self.pose_path, f"c2ws_{step}")
                        plt.savefig(fg_output_path)
                pbar.update()   
    ...
    def generate_poses_delta(self):
        # 生成位姿扰动
        self.cam_rot_delta = torch.nn.Parameter(torch.zeros(self.train_num, 3, device=self.device), requires_grad=True) 
        self.cam_trans_delta = torch.nn.Parameter(torch.zeros(self.train_num, 3, device=self.device), requires_grad=True)
    ...
    def compute_ATE(self, gt, pred):
        ...
        # 每帧欧氏距离 [N]
        errors = torch.norm(align_err, dim=1)
        # RMSE
        ate = torch.sqrt(torch.mean(errors ** 2))
        return ate

    # 旋转损失
    def compute_Rot(self, gt, pred):
        # [N, 3, 3]
        gt_R = gt[:, :3, :3]
        pred_R = pred[:, :3, :3]
        # R_err = R_pred * R_gt^T
        R_err = pred_R @ gt_R.transpose(1, 2)
        # trace: [N]
