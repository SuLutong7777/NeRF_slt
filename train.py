...
class Model_Trainer():
    def __init__(self, sys_param):
        ...
        self.optimizer = self.generate_optimize()
        # 初始化Tensorboard
        if self.tb_mode:
    ...
    def valid(self, step):
        ...
        with torch.no_grad():
            ...
            for i in range(int(num_rays/self.batch_size)+1):
                ...
                data = self.nerf(rays_o, rays_d, mode="test")                                                   # [batch, 3] [batch]
                if self.only_coarse:
    ...
    def generate_optimize(self):
        optimizer = torch.optim.Adam(
            [
                {
                    "params": self.nerf.nerf_hash.parameters(),
                    "lr": self.sys_param["lr_hash"],   # 例如 1e-2 / 1e-3
                },
                {
                    "params": self.nerf.coarse_mlp.parameters(),
                    "lr": self.sys_param["lr_mlp"],    # 例如 1e-3 / 5e-4
                },
                {
                    "params": self.nerf.fine_mlp.parameters(),
                    "lr": self.sys_param["lr_mlp"],    # 例如 1e-3 / 5e-4
                },
            ],
            betas=(0.9, 0.99),
